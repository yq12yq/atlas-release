        ...
        if (ret != null && isIndexableStrAttr) {
            ...
            if (value.length() > INDEXED_STR_SAFE_LEN) {
                ...
                if (trimmedLength < value.length()) {
                    ...
                    LOG.warn("Indexed-String-Attribute: {} length is {} characters, longer than safe-limit {} - attempt #{}", ctx.getAttribute().getQualifiedName(), value.length(), INDEXED_STR_SAFE_LEN, requestContext.getAttemptCount());
                }
            }
        }
